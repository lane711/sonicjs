...
test("should allow unauthenticated user to register without email confirmation", async ({ request }) => {
  ...
  expect(data.email).toEqual(`${e2ePrefix}@test.com`);
...
test("should allow unauthenticated user to register with email confirmation", async ({ request }) => {
  await updateEnvVar(request, "USERS_CAN_REGISTER", "true");
  await updateEnvVar(request, "REQUIRE_EMAIL_CONFIRMATION", "true");
  await updateEnvVar(request, "EMAIL_CONFIRMATION_REDIRECT_URL", "https://sonicjs.com");
  ...
  const response3 = await request.get(`/api/v1/auth/email-confirmation/receive/${userData.emailConfirmationToken}`);
