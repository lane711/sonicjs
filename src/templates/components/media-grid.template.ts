  ...
  if (data.files.length === 0) {
    return `
      <div class="text-center py-12">
        ...
        <p class="mt-1 text-sm text-gray-500">${data.emptyMessage || 'Get started by uploading your first file.'}</p>
      </div>
    `
  }
...
export function renderMediaFileCard(file: MediaFile, viewMode: 'grid' | 'list' = 'grid', selectable: boolean = false): string {
  if (viewMode === 'list') {
    return `
      <div class="media-item bg-white rounded-lg shadow-sm border hover:shadow-md transition-shadow">
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center justify-between">
              ...
              <div class="flex items-center space-x-2">
                <span class="text-sm text-gray-500">${file.fileSize}</span>
                <button 
  ...
  // Grid view
  return `
    <div class="media-item relative bg-white rounded-lg shadow-sm border hover:shadow-md transition-all duration-200 overflow-hidden group">
      ${selectable ? `
