...
export async function loadCollectionConfig(name: string): Promise<CollectionConfig | null> {
  try {
    const module = await import(`../collections/${name}.collection.ts`)

    if (!module.default) {
      console.error(`Collection ${name} does not export a default config`)
      return null
    }

    const config = module.default as CollectionConfig

    // Validate
    if (!config.name || !config.displayName || !config.schema) {
      console.error(`Invalid collection config for ${name}: missing required fields`)
      return null
    }
