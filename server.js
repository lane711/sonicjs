...
const globalService = require('./server/services/global.service')
...
function setupSessionDb (app) {
  ...
  const sessionLengthDays = process.env.SESSION_LENGTH_DAYS
...
function setupPassport (app) {
  passport.use(
    new LocalStrategy(async function (email, password, done) {
      const loginUser = await dalService.userGetByLogin(email, password)
      if (loginUser) {
...
function setupHandlebars (app) {
  const themeDirectory = path.join(__dirname, 'server/themes')
  const partialsDirs = [
...
function setupHandlebarsHelpers () {
  Handlebars.registerHelper({
    ...
    comparerow: function (row, column) {
      const cms = _.camelCase(column.title)
