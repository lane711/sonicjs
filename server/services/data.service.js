...
(function (exports) {
  ...
  (exports.userUpdate = async function (user, sessionID) {
    ...
    (exports.contentTypeUpdate = async function (contentType, sessionID) {
      ...
      const componentExcludingOnesAddedInCustomCode =
        contentType.data.components.filter(
          (c) =>
            c.properties === undefined || c.properties.fromCustomCode !== true
        );
    ...
    (exports.getContentByContentTypeAndTag = async function (
      ...
      if (allOfContentType) {
        let contentByTag = allOfContentType.filter(
          (x) => x.data.tags && x.data.tags.includes(tag.id)
        );
        return contentByTag;
      }
    }),
