...
module.exports = mediaService = {
  startup: async function () {
    ...
    emitterService.on('requestBegin', async function (options) {
      ...
      if (options) {
        const baseUrl = globalService.baseUrl
  ...
  processHeroImage: async function (page) {
    ...
    const jumbotronStyle = 'background:pink;'
  ...
  getMedia: async function (sessionID) {
    ...
    if (storageOption !== 'AMAZON_S3') {
      const mediaFilesList = fileService.getFilesSync('/server/assets/uploads')
    ...
    const sortedMediaRecords = _.sortBy(mediaRecords, (i) =>
  ...
  mediaDelete: async function (id, sessionID) {
    const media = await dataService.getContentById(id, sessionID)
