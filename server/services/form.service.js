...
(function (exports) {
  ...
  (exports.getForm = async function (
    ...
    const data = { viewModel: {}, viewPath: '/server/assets/html/form.html' }
    data.viewModel.onFormSubmitFunction = onFormSubmitFunction
    data.viewModel.formJSON = JSON.stringify(formJSON)
    const formValuesToLoad = content && content.data ? content.data : {}
    ...
    // debugger;
    const formHtml = await dataService.getView(
      '',
      data.viewModel,
      data.viewPath
    )
  ...
  (exports.getFormJson = async function (contentType, content) {
    const name = `${contentType.systemId}Form`
    const settings = await this.getFormSettings(contentType, content)
    const components = await this.getFormComponents(contentType, content)
    const formJSON = {
  ...
  (exports.getFormTemplate = async function () {
    if (isBackEndMode) {
      ...
      const template = await globalService.axiosInstance.get('/html/form.html')
      return template.data
    }
  }),
  (exports.getFormTemplateFileSystem = async function () {
    return new Promise((resolve, reject) => {
      const themeFilePath = '/server/assets/html/form.html'
  ...
  (exports.getFormComponents = async function (contentType, content) {
    const components = contentType.data?.components
