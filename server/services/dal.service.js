...
module.exports = dalService = {
  ...
  userGet: async function (id, sessionID, req) {
    ...
    const session = await dalService.sessionGet(sessionID)
    ...
    const sessionUser = await userRepo.findOne(session.user.id)
  ...
  userGetByLogin: async function (email, password) {
    ...
    if (userByEmail) {
      const passwordHash = await dalService.hashPassword(
        password,
        userByEmail.salt
      )
  ...
  userRegister: async function (email, passwordHash, isAdmin) {
    ...
    if (!user) {
      ...
      const userRecord = await userRepo.save(newUser)
    }
  ...
  userUpdate: async function (userArgs, userSession) {
    ...
    const userRecord = await dalService.userGet(userArgs.id, userSession)
  ...
  contentGet: async function (
    ...
    if (id) {
      const content = await contentRepo.findOne({ where: { id: id } })
      ...
      const content = await contentRepo.findOne({ where: { url: url } })
      dalService.processContent(content, user, req)
      return content
  ...
  contentUpdate: async function (id, url, data, userSession) {
    ...
    const result = await contentRepo.save(content)
  ...
  contentRestore: async function (payload, userSession) {
    ...
    //     payload.url = url;
    //     payload.data = data;
    const result = await contentRepo.save(payload)
    // console.log(result);
  },
  ...
  userRestore: async function (id, url, data, userSession) {
    ...
    const result = await contentRepo.save(data)
    console.log(result)
  },
  ...
  checkPermission: async function (data, user, req) {
    ...
    const contentType = await moduleService.getModuleContentType(
      contentTypeId,
      user,
      req
    )
    ...
    if (contentType.permissions) {
      const view = contentType.permissions.mappings.find(
        (x) => (x = 'view')
      ).view
      if (view === 'admin') {
        data = []
