...
(function (exports) {
  (exports.formatDates = async function (data, dateOnly = false) {
    const dateFormat = dateOnly ? 'YYYY-MM-DD' : 'YYYY-MM-DD, h:mm:ss a'

    data.forEach((element) => {
      if (element.data) {
        if (element.updatedOn) {
          const day = moment(element.updatedOn)
          element.updatedOnFormatted = moment(day).format(dateFormat)
        }
        if (element.createdOn) {
          const day = moment(element.createdOn)
    ...
    data.forEach((element) => {
      if (element.data) {
        if (!element.data.title) {
          ...
          const rawBody = this.stripHtmlTags(sourceCopy)
          if (rawBody) {
