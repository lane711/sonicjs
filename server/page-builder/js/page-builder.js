...
function setupUIHovers () {
  $('.pb-section').on({
    mouseenter: function () {
      const sectionId = getParentSectionId($(this))
...
async function setupSectionBackgroundEvents () {
  $('.section-background-editor button').on('click', async function () {
    const backgroundSetting = $(this).data('type')
...
async function showBackgroundTypeOptions (backgroundSetting, sectionId) {
  $('[id^=background-]').hide()
  const selector = `[id='background-${backgroundSetting}'],[data-id='${sectionId}']`
  $(selector).show()
}
...
async function addSection () {
  ...
  const row = await generateNewRow()
  // rows
  const rows = [row]
  ...
  const s1 = await createInstance(section)
  ...
  const updatedPage = await editInstance(page)
...
async function saveSection () {
  const sectionData = jsonEditor.get()
...
async function generateNewRow () {
  const col = await generateNewColumn()

  const row = { class: 'row', columns: [col] }
...
async function addRow () {
  const row = await this.generateNewRow()

  const section = await dataService.getContentById(currentSectionId)
  section.data.rows.push(row)
  editInstance(section)
...
async function addColumn () {
  ...
  const column = await generateNewColumn()
...
async function createInstance (
  ...
  // debugger;
  const entity = await dataService.contentCreate(payload)

  if (entity && entity.contentTypeId === 'page') {
    const isBackEnd = globalService.isBackEnd()
    if (isBackEnd) {
...
async function onContentTypeRawSave () {
  // debugger;
  const contentType = jsonEditorRaw.get()
  ...
  $('textarea.wysiwyg-content').tinymce({
    ...
    images_upload_handler: function (blobInfo, success, failure) {
      ...
      xhr.onload = function () {
        ...
        const file = json.result.files.file[0]
        const location = `/api/containers/${file.container}/download/${file.name}`
        if (!location) {
...
function setupJsonEditorContentTypeRaw () {
  const containerRaw = document.getElementById('jsoneditorRaw')
...
function loadJsonEditor () {
  const json = currentSectionRecord
  jsonEditor.set(json)

  // get json
  const editor = jsonEditor.get()
}

function setupJsonRawSave () {
  $('#saveRawJson').on('click', function () {
    debugger
    const rawData = jsonEditorRaw.get()
...
async function getImageList () {
  const imageList = await dataService.getFiles()
...
async function saveWYSIWYG () {
  ...
  const content = $('textarea.wysiwyg-content').html()

  // update db
  const block = await dataService.getContentById(id)
...
async function editModule (sessionID) {
  ...
  const data = await dataService.getContentById(currentModuleId)
...
async function deleteModule () {
  ...
  const confirmDeleteButton = `<div class="btn-group">
  ...
  const dataPreview = `<div class="delete-data-preview""><textarea>${JSON.stringify(
    data,
    null,
    4
  )}</textarea></div>`
...
async function deleteModuleConfirm (deleteContent = false) {
  ...
  const moduleDiv = $(`.module[data-id='${currentModuleId}'`)
  const {
    isPageUsingTemplate,
  ...
  // debugger;
  const source = await getModuleHierarchy(moduleDiv)
...
function getPageTemplateRegion (page, sourceColumn, destinationColumn) {
  ...
  if (isPageUsingTemplate) {
    // debugger;
    const sourceRegionModule = $(sourceColumn.children).filter(function () {
...
async function addModuleToColumn (submission) {
  const entity = processContentFields(submission.data)
...
async function submitContent (
  ...
  const entity = submission.data ? submission.data : submission
...
async function postProcessNewContent (content) {
  ...
  if (content.contentType == 'page') {
    if (content.includeInMenu) {
      ...
      // await editInstance(entity);
      const mainMenu = await getContentByContentTypeAndTitle('menu', 'Main')
      const menuItem = {
        url: content.url,
        title: content.name,
        active: true,
...
async function setupACEEditor () {
  ...
  $('#save-global-css').click(async function () {
    const cssContent = editor.getSession().getValue()
...
async function setupDropZone () {
  ...
  const myDropzone = $(document.body).dropzone({
    url: '/dropzone-upload',
    addRemoveLinks: true,
    maxFilesize: 100,
    dictDefaultMessage:
...
async function beatifyACECss () {
  if (typeof ace !== 'undefined') {
    ...
    const editor = ace.edit('editor') // get reference to editor
    beautify.beautify(editor.session)
  }
}

async function setupSortable () {
  const columnsList = $('main .pb div[class^="col"]')
  // TODO: limited this to only columns that are managed by page builder
  const columns = jQuery.makeArray(columnsList)
...
async function getModuleHierarchy (element) {
  const sourceSectionHtml = $(element)[0].closest('section')
  const sourceSectionId = sourceSectionHtml.dataset.id
  const sourceRow = $(element)[0].closest('.row')
  const sourceRowIndex = $(sourceRow).index()
  const sourceColumn = $(element)[0].closest('div[class^="col"]')
  const sourceColumnIndex = $(sourceColumn).index()
...
async function updateModuleSort (shortCode, event) {
  ...
  const moduleBeingMovedId = event.item.dataset.id
  const sourceColumn = $(event.from)[0].closest('div[class^="col"]')
  const destinationColumn = $(event.to)[0].closest('div[class^="col"]')
  ...
  // source
  const source = await getModuleHierarchy(event.from)

  // destination
  const destinationSectionHtml = $(event.to)[0].closest('section')
  const destinationSectionId = destinationSectionHtml.dataset.id
  const destinationRow = $(event.to)[0].closest('.row')
  const destinationRowIndex = $(destinationRow).index()
  const destinationColumnIndex = $(destinationColumn).index()

  // get destination list of modules in their updated sort order
  let destinationModules
  const destinationModuleFilter = isPageUsingTemplate
    ? "[data-template-region='true']"
    : '.module'
  ...
  destinationModules = $(destinationColumn)
    ...
    .toArray()
    .map(function (div) {
      const shortCodeData = { id: div.dataset.id, module: div.dataset.module }
      return shortCodeData
    })
...
function isEditMode () {
  const isEditMode = Cookies.get('showSidebar')
  return isEditMode
}
...
async function setupAdminMediaFormImage () {
  if (window.location.href.indexOf('admin/content/edit/media/') > 0) {
    const fileName = $('input[name="data[file]"]').val()
    if (fileName) {
      if ($('#fileStorage').val() == 'AMAZON_S3') {
        const storageBase = $('#fileStorageBase').val()
