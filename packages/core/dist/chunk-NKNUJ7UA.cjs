...
function bootstrapMiddleware(config = {}) {
  return async (c, next) => {
    ...
    try {
      ...
      const migrationService = new chunkLJB5SPNC_cjs.MigrationService(c.env.DB);
      await migrationService.runPendingMigrations();
      console.log("[Bootstrap] Syncing collection configurations...");
      try {
        await chunkNAYD76QF_cjs.syncCollections(c.env.DB);
      } catch (error) {
        console.error("[Bootstrap] Error syncing collections:", error);
      }
      if (!config.plugins?.disableAll) {
        console.log("[Bootstrap] Bootstrapping core plugins...");
        const bootstrapService = new chunkNAYD76QF_cjs.PluginBootstrapService(c.env.DB);
...
//# sourceMappingURL=chunk-NKNUJ7UA.cjs.map
//# sourceMappingURL=chunk-NKNUJ7UA.cjs.map