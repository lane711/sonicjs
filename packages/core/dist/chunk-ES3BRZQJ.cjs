...
var TelemetryService = class {
  ...
  constructor(config) {
    this.config = {
      ...chunk7KCDFDRI_cjs.getTelemetryConfig(),
      ...config
    };
  ...
  async trackInstallationFailed(error, properties) {
    await this.track("installation_failed", {
      ...properties,
      errorType: chunk7KCDFDRI_cjs.sanitizeErrorMessage(error)
    });
  }
  /**
  ...
  async trackPageView(route, properties) {
    await this.track("page_viewed", {
      ...properties,
      route: chunk7KCDFDRI_cjs.sanitizeRoute(route)
    });
  }
  /**
  ...
  sanitizeProperties(properties) {
    ...
    for (const [key, value] of Object.entries(properties)) {
      ...
      if (key === "route" && typeof value === "string") {
        sanitized[key] = chunk7KCDFDRI_cjs.sanitizeRoute(value);
        continue;
      }
      if (key.toLowerCase().includes("error") && typeof value === "string") {
        sanitized[key] = chunk7KCDFDRI_cjs.sanitizeErrorMessage(value);
        continue;
      }
...
function createInstallationIdentity(projectName) {
  const installationId = chunk7KCDFDRI_cjs.generateInstallationId();
  const identity = { installationId };
  if (projectName) {
    identity.projectId = chunk7KCDFDRI_cjs.generateProjectId(projectName);
  }
  return identity;
}
...
//# sourceMappingURL=chunk-ES3BRZQJ.cjs.map
//# sourceMappingURL=chunk-ES3BRZQJ.cjs.map